...
| `DRY_MULTIPLIER` | `0` (off) | no | DRY sampler strength (`0` = off). **Off by default: in agentic/coding use it truncated repeated file paths** (the model kept emitting `src/…/file.py`, DRY penalized the repeat and cut the path mid-token → broken tool calls). Only enable for pure prose where verbatim-repetition loops are the main risk. |
